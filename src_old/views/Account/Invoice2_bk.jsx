...
export default function Invoice() {
  ...
  const getTotalInfo = (add) => {
    // let gstValue = parseInt(add.gst) / 100;
    // let gsttotal = parseInt(add.grandtotal * gstValue) / 2;
  ...
  const performaEdit = () => {
    ...
    setValue(
      "date",
      moment(location.state.main.date).toISOString().slice(0, 10)
    );
    setValue(
      "delivery_date",
      moment(location.state.main.delivery_date).toISOString().slice(0, 10)
    );
    setValue("buyer_order_no", location.state.main.buyer_order_no);
    setValue(
      "buyer_order_date",
      moment(location.state.main.buyer_order_date).toISOString().slice(0, 10)
    );
  ...
  const onsubmit = (data) => {
    ...
    let postData = {
      ...
      delivery_date: moment(data.delivery_date).toISOString().slice(0, 10),
                      ...
                      <td>
                        <SupplierSearch
                          ...
                          onChangeData={(data) => {
                            ...
                            HandleStocks(item.product_id, data.supplier_idd);
                            if (descCheck) {
                      ...
                      <td>
                        <SupplierSearch
                          ...
                          onChangeData={(data) => {
                            ...
                            productChange(data.supplier_idd, "supplier_id"); //to sore supplier_id in databse
                            HandleStocks(add.product_id, data.supplier_idd);
                            if (descCheck) {
                              let contactdesc = data.unit_qty + data.sub_uom;
