...
import { deleteAPI, getAPI, postFormDataAPI } from "../../../apiServices/AxiousFetch";
...
export default function PurchaseLedgerTransport() {
  ...
  const handleGet = (id) => {
    console.log(id, "qwerkyui");
    getAPI(apinames.GET_PURCHASE_TRANSPORT + id).then((response) => {
  ...
  const onsubmitdata = async (data) => {
    ...
    try {
      ...
      newFormData.append("invoice_id", location.state.id);
      ...
      newFormData.append(
        "invoice_date",
        moment(location.state.invoice_date).toISOString().slice(0, 10)
      );
      console.log(newFormData, "data");
      const response = await postFormDataAPI(
        apinames.POST_PURCHASE_TRANSPORT,
        newFormData
      );
    ...
    const deleteHandle = (id) => {
      ...
      if (confirmed) {
        try {
          deleteAPI(apinames.DELETE_PURCHASE_TRANSPORT + id).then((response) => {
