...
import FunctionAlert from "../alert/DispatchThroughAlert";
import { Refresh } from "@mui/icons-material";

const SignupSchema = yup.object({
  name: yup
    .string()
    .required("Dispatch Through  Name required field")
    
});

export default function DispatchThrough() {
  ...
  useEffect(() => {
    getAPI(apinames.GET_Dispatch_Through)
      .then((response) => {
  ...
  const onSubmit = (data) => {
    ...
    if (!isPresent) {
      ...
      postAPI(apinames.POST_Dispatch_Through, postData)
      .then((response) => {
  ...
  const onSubmitEdit = async (data) => {
    try {
      const isPresentResponse = await getAPI(apinames.GET_Dispatch_Through);
      ...
      if (isPresent) {
        ...
        const response = await PutAPI(
          apinames.PUT_Dispatch_Through+ eedit,
          editData
        );
      ...
      <div>
        <Modal show={show} onHide={reportingClose}>
          <Modal.Header closeButton>
            <Modal.Title>Add Dispatch Through</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div>
              <form key={1} onSubmit={handleSubmit(onSubmit)}>
                <table className="table table-borderless">
                  <thead>
                    <tr>
                      <th>Dispatch  Name:  <span className="text-danger">*</span>{" "}</th>
                      <td>
                        <input type="text" maxLength={50} {...register("name")} />{" "}
                        <br />
      ...
      <div>
        <div>
          <Modal show={open} onHide={editClose}>
            <Modal.Header closeButton>
              <Modal.Title>Edit Dispatch Through</Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <div>
