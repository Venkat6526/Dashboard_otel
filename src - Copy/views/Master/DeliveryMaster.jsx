...
import FunctionAlert from "../alert/DeliveryMasterAlert";
...
export default function DeliveryExecutive() {
  ...
  useEffect(() => {
    getAPI(apinames.GET_DELIVERY_EXECUTIVE)
      .then((response) => {
  ...
  const onSubmit = (data) => {
    ...
    if (!isPresent) {
      const postData = {
        ...
        delivery_status: data.delivery_status ? "Active" : "Inactive",
      };
      console.log(postData, "jgfgtfg");
      // console.log(postData, "dataaaa");
      postAPI(apinames.POST_DELIVERY_EXECUTIVE, postData)
        .then((response) => {
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        ...
        deleteAPI(apinames.DELETE_DELIVERY_EXECUTIVE + id).then((response) => {
  ...
  const checkIssue = (subj, role) => {
    return get.some((o) => o.mobile.toLowerCase() === subj.toLowerCase());
  };
      ...
      <div>
        <Modal show={show} onHide={deliveryClose}>
          <Modal.Header closeButton>
            <Modal.Title>Add Delivery Executive</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div>
                ...
                <table className="table table-borderless">
                  <thead>
                    <tr>
                      <th>
                        {" "}
                        Delivery Name: <span className="text-danger">
                          *
                        </span>{" "}
                      </th>
                      <td>
