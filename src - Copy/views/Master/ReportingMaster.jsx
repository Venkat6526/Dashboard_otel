...
import FunctionAlert from "../alert/ReportingMasterAlert";
...
const SignupSchema = yup.object({
  ...
  members_role: yup
    .string()
    .oneOf(
      ["Employee", "Vendor", "Hotel_Owner", "Supplier", "members"],
      "Role is required"
    ),
...
export default function ReportingMaster() {
  ...
  const editDisplay = (data) => {
    ...
    setValue("members_role", data.members_role);
  ...
  useEffect(() => {
    getAPI(apinames.GET_REPORTING_MASTER)
      .then((response) => {
  ...
  const onSubmit = (data) => {
    ...
    let isPresent = false;
    isPresent = checkIssue(data.name, data.members_role);
    ...
    if (!isPresent) {
      ...
      postAPI(apinames.POST_REPORTING_MASTER, postData)
        .then((response) => {
  ...
  const onSubmitEdit = async (data) => {
    try {
      const isPresentResponse = await getAPI(apinames.GET_REPORTING_MASTER);
      ...
      const isPresent = isPresentData.some(
        (item) =>
          item.name === data.name &&
          item.members_role === data.members_role &&
          item.id !== eedit
      );
      ...
      if (isPresent) {
        ...
        const response = await PutAPI(
          apinames.PUT_REPORTING_MASTER + eedit,
          editData
        );
        let alert_data;
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmed) {
      try {
        ...
        deleteAPI(apinames.DELETE_REPORTING_MASTER + id).then((response) => {
  ...
  const checkIssue = (subj, role) => {
    return get.some((o) => o.name.toLowerCase() === subj.toLowerCase()&&o.members_role.toLowerCase() === role.toLowerCase());
  };
  return (
    <div>
      <div>
            ...
            <div>
              <form key={1} onSubmit={handleSubmit(onSubmit)}>
                <table className="table table-borderless">
                  <thead>
                    <tr>
                      <th>Reporting Subject:  <span className="text-danger">*</span>{" "}</th>
                      <td>
                        <input
                          type="text"
                          maxLength={100}
                    ...
                    <tr>
                      ...
                      <td>
                        {" "}
                        <select className="btn btn-outline-dark"{...register("members_role")}>
                        ...
                        <br />
                        <span className="text-danger">
                          {errors.members_role && errors.members_role.message}
                        </span>
                      </td>
                    </tr>
                    <tr>
                    ...
                    <thead>
                      ...
                      <tr>
                        ...
                        <td>
                          <select
                            type="number"
                            value={values.members_role}
                            onChange={(e) => {
                              setValue("members_role", e.target.value);
                            }}
                            {...register2("members_role")}
                          >
                            <option value="select">Select</option>
                            <option value="Employee">Employee</option>
                            <option value="Hotel_Owner">Hotel Owner</option>
                          ...
                          <span className="text-danger">
                            {errors2.members_role &&
                              errors2.members_role.message}
                          </span>
                          <br />
                        </td>
                      </tr>
